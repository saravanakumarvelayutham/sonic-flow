...
/**
 * YouTube Token Refresh Endpoint
...
export async function POST() {
  try {
    ...
    // await refreshAccessToken('youtube', session.refreshToken)
...
export async function youtubeTokenRefresh() {
  try {
    const cookieStore = await cookies()
    const sessionCookie = cookieStore.get('sonicflow_session')?.value
    ...
    if (session.provider !== 'youtube') {
      throw new Error('Not a YouTube session')
    }
